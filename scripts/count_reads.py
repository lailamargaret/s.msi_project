...
def count(runfile_loc, locus, run_quality_threshold = 0.1, flank_length = 7,  flank_mismatch = 2, print_full = False, plot = False, show_reads = False, return_mms = False):
	...
	#filter for reads that contain both flanking regions
        for read in reads:
                ...
                cropped = []
                if return_mms:
                        matched1, matched_front, front_mm = match.approximate_match(read, front_flank, flank_mismatch, return_mm = True)
                else:
                        matched1, matched_front = match.approximate_match(read, front_flank, flank_mismatch)
                if matched1:
                ...
                if cropped != []:
                        if return_mms:
                                matched2, matched_back, back_mm = match.approximate_match(cropped[1], back_flank, flank_mismatch, return_mm = True)
                        else:
                                matched2, matched_back = match.approximate_match(cropped[1], back_flank, flank_mismatch)
                        if matched2:
